...
db = MySQL.connect(user = 'root', passwd = 'root', host = 'localhost', db = 'knapsack')
cur = db.cursor()
cur.execute('select * from data')
things = cur.fetchall()
things = [i[1:4] for i in things]
"""
...
def load_knapsack(things,knapsack_cap):
   ...
   """        
    ...
    things = {0:(1,2), 1:(3, 4), 2:(5, 6), 3:(1,4), 4:(5,10)}
    knapsack_cap = 6
    ...
    w = [item_list[i][1][1] / item_list[i][1][0] for i in range(len(item_list))]
    
    #item_list = [item_list[i][1][1] * w[i] for i in range(len(item_list))]
 #Input:
